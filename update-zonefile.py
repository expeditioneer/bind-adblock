...
def get_blocklist_content(url: str) -> List[str]:
    ...
    if 'last-modified' in r.headers:
        last_modified_source = parsedate(r.headers['last-modified'])
    else:
        last_modified_source = datetime.now().astimezone()
        ...
        try:
            if r.status_code == 200:
                ...
                with cache.open('w') as file:
                    ...
                    # DNS is case insensitive; normalise so the cache is canonical
                    data = sorted(element.lower() for element in data)
